...
0.4.0 as released on 2026-07-15.
...
[Version 0.4.0](https://github.com/AurelienNicosiaULaval/lmmix/releases/tag/v0.4.0),
published on 2026-07-15, introduced connected-component block factorization
for the likelihood, cached Satterthwaite derivatives, simulation, model
updating, prediction intervals, partial random-effect prediction, known
precision weights, offsets, and custom contrasts. It also standardized the
`ranef()` return type and normal `print()` output.
...
- parametric-bootstrap likelihood-ratio tests for boundary-sensitive model
  comparisons;
- marginal simulation, model updating, prediction intervals, known relative
  precision weights, offsets, and custom fixed-effect contrasts.
...
- Residual variance strata estimated from the data, analogous to
  `nlme::varIdent()`, are not implemented. `weights` represent known relative
  precisions and are not estimated.

### Computation and scale

- Independent connected covariance components are assembled and factorized
  separately during likelihood evaluation.
- The fitted object still stores a complete dense marginal covariance matrix,
  and some inference calculations remain dense.
- Fully crossed designs can form one large connected component. Practical cost
  depends primarily on the largest component, the covariance parameter count,
...
- Prediction confidence intervals condition on included empirical random
  effects. Prediction intervals add residual variance but not BLUP estimation
  uncertainty.
...
- Implement estimability-aware handling of aliased fixed-effect columns and
  define its consequences for type III tests and marginal means.
- Evaluate estimated residual variance strata with an explicit covariance
  parameterization and validation against `nlme` or PROC MIXED.

### Computation

- Evaluate a sparse Woodbury formulation for large crossed random-effect
  designs.
- Derive and test analytic covariance derivatives for supported residual and
  random-effect structures, including their use in Kenward-Roger inference.
