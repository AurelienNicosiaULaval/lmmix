...
example in Section 28.3 of Milliken and Johnson (2009). The data contain
153 rows, including 28 missing responses, so the reference model uses
125 observations.
...
of freedom with stored SAS PROC MIXED benchmarks for this specification.
The exact SAS statements are given in the validation vignette. PROC
MIXED algorithms and syntax are documented in the current SAS/STAT
User’s Guide:
...
Degrees of freedom are compared at the precision stored with the
benchmark. Overlapping model classes are independently compared with
`nlme`, `lmerTest`, and `mmrm`.
